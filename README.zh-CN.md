...
- 2026-09-09 · v0.5.4：修复页面更新标题或替换标题节点后，环境标题前缀偶尔消失的问题。
- 2026-08-31 · v0.5.3：修复新增分组后未立即保存的问题。
...
颜色滤镜支持“自动识别 / 主体换色 / 背景换色”。自动识别会保留浅色背景、修改主体，或修改连续的深色/彩色背景、保留内部文字；透明底图标修改主体并保留透明度。预览同时展示透明底、白底和彩底白字三种 16×16 样例。复杂或多色图标可能识别不准，可手动选择区域；背景换色不会为透明区域增加底色。

在环境设置的“标签页图标”区域选择保持原图、颜色滤镜或自定义图片。滤镜支持独立颜色和强度，保留原图的明暗和透明度；预览使用示例图标，实际页面使用网站原图标。自定义图片支持 ICO、PNG、JPEG、WebP，最大 2 MB，转换成 64×64 PNG 后随配置本地保存，并支持复制和导入导出。

点击保存后，已打开的匹配页面会更新。切换至不匹配的地址、禁用环境或选择保持原图时恢复网站图标。旧配置默认保持原图。原图读取失败时保留网站图标；需要登录才能读取的图标可能无法使用滤镜，可以改为上传图片。浏览器内部页面不支持此功能。

安装、启动或保存配置时，扩展会检查已打开的匹配页面并补加载缺失脚本，无需刷新网页。此功能使用 scripting 权限；浏览器内部页面和不允许扩展运行的页面除外。

...
- 2026-09-13 · v0.6.0：新增标签页配置功能，支持自定义浏览器标签页图标、标题前缀和标题；修复了一些问题。
- 2026-09-10 · v0.5.5：新增配置区块焦点呼吸高亮，在编辑时清晰标示当前配置范围。
...
每个环境都可以配置 `tabDisplay`。顶部开关同时控制标题和图标修改；关闭后会恢复网站原始值，但不会删除已经保存的配置。
...
图标来源支持 `emoji`、`preset` 和 `upload`。预置图标目前提供方形（`solid-rounded-square`）和圆形（`solid-circle`）两种形状，共用同一套色值配置。上传原图不能超过 2 MB，图片会在本地转换成小于 256 KB 的方形 PNG Data URL，并随配置一起导入导出，不会上传或远程抓取图片。`override` 留空时保留网页当前标题，`prefix` 会按输入内容原样拼接。只要 `prefix` 当前为空，聚焦输入框就会填入 `[]`；用户清空后，下次聚焦仍会再次提供模板。
